...
import { ArrowLeft, BarChart3, BrainCircuit, Check, CircleHelp, FileText, Fingerprint, Gauge, LayoutDashboard, Loader2, LogIn, MessageSquareText, RotateCcw, Sparkles, Target, UploadCloud, WandSparkles } from "lucide-react";
...
export default function Dashboard() {
  ...
  return <main className="workspace">
    <header className="workspace-header">
      <div className="workspace-header-left"><Link className="brand" href="/"><span className="brand-mark"><Fingerprint /></span><span>Am I a Good Match?</span></Link><div className="workspace-context"><strong>Match workspace</strong><span>Evidence-based career intelligence</span></div></div>
      <div className="workspace-header-right"><Button className="secondary" onClick={resetAnalysis}><RotateCcw /> New analysis</Button></div>
    </header>
    <div className="workspace-layout">
      <aside className="workspace-sidebar"><p className="side-label">Workspace</p><a className="side-link active" href="#analysis"><LayoutDashboard /><span>Role analysis</span></a><a className="side-link" href="#report"><BarChart3 /><span>Match report</span></a>{analysis && <a className="side-link" href="#coach"><MessageSquareText /><span>Resume coach</span></a>}<div className="sidebar-spacer" /><Link className="side-link" href="/"><ArrowLeft /><span>Back to site</span></Link>{token && <div className="account-chip"><strong>{currentUser?.full_name || "Signed in"}</strong><span>{currentUser?.email || email}</span><button className="signout" onClick={signOut}>Sign out</button></div>}</aside>
      <section className="workspace-main" id="analysis">
        ...
        <div className="metrics" id="report"><Stat label="Overall signal" value={analysis?`${matchScore}%`:"—"} percent={matchScore}/><Stat label="Skill alignment" value={analysis?`${skillScore}%`:"—"} percent={skillScore}/><Stat label="Experience fit" value={analysis?`${experienceScore}%`:"—"} percent={experienceScore}/></div>
        ...
        {analysis ? <>
          ...
          <Panel className="coach" id="coach"><div className="surface-head"><div><span className="kicker">Grounded in your resume</span><h2>Ask the resume coach</h2><p>The coach retrieves relevant evidence from your uploaded resume before answering.</p></div><BrainCircuit /></div><div className="coach-form"><Input aria-label="Question for resume coach" placeholder="What is the strongest evidence I have for this role?" value={coachQuestion} onChange={e=>setCoachQuestion(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")void handleCoach();}}/><Button disabled={coachBusy||!coachQuestion.trim()} onClick={handleCoach}>{coachBusy?<Loader2 className="spin"/>:<MessageSquareText/>} Ask coach</Button></div>{coachAnswer&&<div className="coach-answer">{coachAnswer}</div>}</Panel>
